...
namespace gpm_slam{
    NewLineSubscriber::NewLineSubscriber(const PointSet point_data)
    {
        originpoint=point_data;
        msg_callback();
        
    };
    
    void NewLineSubscriber::ParseData(std::deque<LineData>& line_data_buff)
    {
    ...
    void NewLineSubscriber::NewLineDataInit()
    {
        /*clock_t startTime,endTime;
        startTime = clock();*/
    ...
    void NewLineSubscriber::FitLineWithPoint()
    {
           //clock_t startTime,endTime;
           //startTime = clock();
           int line_num_=0;
           int point_size=originpoint.size();
           static int count=0;
           std::cout<<"points size"<<point_size<<std::endl;
           for (int i=0;i<point_size;i++)
           {
               int seg_state_=0;
               float x_temp_=originpoint[i].x;
               float y_temp_=originpoint[i].y;
            ...
            if(count==1500)
            {
                ...
                for(int i=0;i<point_size;i++){
                    outfile<<originpoint[i].x<<" "<<originpoint[i].y<<"\n";
                }
                outfile.close();
            };
    }; 

    void NewLineSubscriber::WhetherSegmentIsValid(LineData line_data_)
    {
        //clock_t startTime,endTime;
        //startTime = clock();
        ...
        for (int i=0;i<MAXSEGMENTS;i++)
        {
            ...
            if((init_segs_[i].count_>count_threhold_)&&(length_>=length_limit_))
            {
                if(init_segs_[i].grad_!=__DBL_MAX__)
                {
                    ...
                    segs_temp_.start_point.x=x_mean_-length_/(2*sqrt(pow(init_segs_[i].grad_,2)+1));
                    segs_temp_.start_point.y=y_mean_-init_segs_[i].grad_*length_/(2*sqrt(pow(init_segs_[i].grad_,2)+1));
                    segs_temp_.end_point.x=x_mean_+length_/(2*sqrt(pow(init_segs_[i].grad_,2)+1));
                    segs_temp_.end_point.y=y_mean_+init_segs_[i].grad_*length_/(2*sqrt(pow(init_segs_[i].grad_,2)+1));
                }
                else if ((init_segs_[i].grad_==__DBL_MAX__)&&(init_segs_[i].invgrad_!=0))
                {
                    double x_mean_ = (init_segs_[i].x_start_ + init_segs_[i].x_end_) / 2;
				    double y_mean_ = (init_segs_[i].y_start_ + init_segs_[i].y_end_) / 2;
                    segs_temp_.start_point.x=x_mean_-init_segs_[i].invgrad_*length_/(2*sqrt(pow(init_segs_[i].invgrad_,2)+1));
                    segs_temp_.start_point.y=y_mean_-length_/(2*sqrt(pow(init_segs_[i].invgrad_,2)+1));
                    segs_temp_.end_point.x=x_mean_+init_segs_[i].invgrad_*length_/(2*sqrt(pow(init_segs_[i].invgrad_,2)+1));
                    segs_temp_.end_point.y=y_mean_+length_/(2*sqrt(pow(init_segs_[i].invgrad_,2)+1));
                }
    ...
    void NewLineSubscriber::msg_callback()
    {
        LineData line_data_;
        ...
        {
            outfile_2<<originpoint[i].x<<" "<<originpoint[i].y<<"\n";
        }
        outfile_2.close();*/
        //cnt_++;
        NewLineDataInit();
