...
int main(int argc, char *argv[]) {
    ...
    ros::init(argc, argv, "test_linemap_node");
    ros::NodeHandle nh;
	...
	gt_.open("./gt.txt", ios::out | ios::trunc);
	est_.open("./estwithgtasprioer.txt", ios::out | ios::trunc);
	//生成初始化CSM地图
	...
	/*imwrite("/home/tanqingge/map_withmerge.png", matchsrc);
	WhetherLineIsSameTotal(ptr_setbefore,ptr_setafter);
	for(int i=0;i<ptr_setafter->size();i++)
	{
		Point pts, pte;
		pts.x = (*ptr_setafter)[i].start_point.x * 50 + 1000;
		pts.y = (*ptr_setafter)[i].start_point.y * 50 + 1500;
		pte.x = (*ptr_setafter)[i].end_point.x * 50 + 1000;
		pte.y = (*ptr_setafter)[i].end_point.y * 50 + 1500;
		line(Img, pts, pte, Scalar(255, 255, 0), 1, 8, 2);
                        
	}
	imwrite("/home/tanqingge/map_withmergeafter.png", Img);
